...
不解释、不总结。第一个字符必须是 L 或 I。""",
    ...
    'liti': """转写这一页中的「例题」。这是《义务教育数学课程标准（2022年版）》附录中的实例。

每个例题的版式是：
    例 4  用不同符号表示变化规律     ← 例号 + 标题（加粗一行）
    在下列横线上填上合适的数字…      ← 题干，可能多段
    【说明】启发学生在解决问题的过程中…  ← 教学说明

输出规则（每个例题输出一行，四段，用制表符分隔）：
- 例号<TAB>标题<TAB>题干<TAB>说明
- 例号只写阿拉伯数字（「例 4」输出 4）
- 题干与说明里的换行一律换成空格，压成一行
- 【说明】之后的内容放第四段；没有【说明】时第四段留空
- 表格、算式、图形无法转写的，在原处写 [表] [图] 占位，不要跳过
- 若本页是上一页某例题的续页（页首不是「例 N」开头），输出一行 CONT<TAB><TAB>续页内容<TAB>
- 不要输出页眉、页脚、页码
不解释、不总结。第一个字符必须是数字或 C。""",

    'neirong': """把这一页【内容要求】里的要求，逐条改写成**可判定的学生能力断言**。

这是《义务教育课程标准（2022年版）》某科的「内容要求」。它的原文形如：
    通过了解元谋人、蓝田人、北京人等旧石器时代的人类及其文化遗存，知道中国境内
    原始社会时期的人类活动；知道甲骨文是已知最早的汉字；了解都江堰工程……

改写规则（**这是本任务的核心，逐条照做**）：
- 原文动词是「知道 / 了解 / 认识」时，把它改写成「能说出……」的形式，且**必须给出
  一个有标准答案的问题所对应的命题**：
    「知道甲骨文是已知最早的汉字」→ 能说出已知最早的汉字是甲骨文
    「了解都江堰工程」→ 能说出都江堰工程的作用是防洪灌溉
- **改写不许凭空添内容。** 断言里的事实必须能在本页原文里找到依据。原文没给出
  具体事实、只给了主题（如「知道原始社会时期的人类活动」）时，**跳过这一条**，
  不要编。宁可少抽，不要编。
- 一条断言只承载一个可判定的点。原文一句话里塞了三件事就拆成三条。
- 断言长度控制在 12–40 字。
- 「感受 / 体会 / 认同 / 树立」这类无法判定的，**整条跳过**。
- **只抽学生要掌握的学科内容，不抽关于课标这份文件本身的话。** 原文里讲
  「本课程内容包括六个板块」「本学段设置三个主题」「教学中应注意……」的，
  整条跳过 —— 那是课程编排说明，不是学生能力。

输出规则（每行三段，制表符分隔）：
    断言<TAB>原文依据<TAB>学段
- 原文依据是本页支撑该断言的原句片段（照抄，不改写）
- 学段写 G1-2 / G3-4 / G5-6 / G7-9 之一；本页没说就留空
- 不要输出页眉页脚页码、标题行、导语段
不解释、不总结。第一个字符必须是「能」。""",
...
def parse(kind, text):
    ...
    for raw in text.strip().split('\n'):
        ...
        if kind == 'buguize':
            ...
            continue
        if kind == 'neirong':
            if parts and parts[0].startswith('能'):
                v = (parts + ['', ''])[:3]
                rows.append(('@' + str(len(rows)), tuple(v)))
            continue
        if kind == 'liti':
            if parts and (parts[0].isdigit() or parts[0] == 'CONT'):
                v = (parts + ['', '', ''])[:4]
                rows.append((v[0] if v[0].isdigit() else '@' + str(len(rows)), tuple(v)))
            continue
        if kind == 'yufa':
