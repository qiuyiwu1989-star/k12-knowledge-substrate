...
# 学科配色。**按族群定色相，族内只变明度与饱和度。**
#
# 2026-08-28 重做。旧表只有 14 个学科 —— 全是义务教育那批，
# 加高中 20 科时没人补，于是 **943 个点（占全图 26%）落到 '#888' 灰**：
# 通用技术 260、西班牙语 142、音乐 114、信息技术 99、思想政治 89……
# 截图里那一大团灰不是设计，是缺色。
#
# 补的时候没有再加十个彩虹色。旧表 14 个色相铺满 339°、相邻平均差 26°，
# 24 个只会更糟 —— 那是「用完了颜色」，不是「用对了颜色」。
# 对照 Marble：8 个学科、实际只有 3 个色系（蓝 / 红粉 / 米白），
# 所以它的图一眼能看出结构，而我们的是一团彩虹噪点。
#
# 改成六个族，族内共享色相、只拉开明度：
#   语言 玫红 · 数理 蓝 · 科学 青 · 人文 琥珀 · 艺体 紫 · 技术 石青
# 这样「同族」在图上自动成片，跨族的距离才有意义。
COLORS = {
    # 语言 —— 玫红族 h≈345
    '语文': '#ff5d73', '英语': '#ff8fa3', '日语': '#e8607d',
    '俄语': '#c9455e', '德语': '#ffb3c1', '法语': '#d96a86', '西班牙语': '#ff7a96',
    # 数理 —— 蓝族 h≈215
    '数学': '#4a9eff', '物理': '#6b7dff', '化学': '#7fb3ff',
    # 科学 —— 青族 h≈180
    '科学': '#22b8cf', '生物学': '#3ecfb2', '地理': '#5fd6d0',
    # 人文 —— 琥珀族 h≈42
    '历史': '#f0a83c', '道德与法治': '#fcc419', '思想政治': '#d99420',
    # 艺体 —— 紫族 h≈285
    '艺术': '#c58cf5', '音乐': '#a97bf0', '美术': '#dba6ff', '体育与健康': '#8f6fd6',
    # 技术 —— 石青族 h≈195
    '信息科技': '#5b9bb5', '信息技术': '#7ab3c9', '通用技术': '#3f7f99', '劳动': '#9ccbd9',
}
W, H = 2000, 1500
...
function show(n, push = true) {
  ...
  history.replaceState(null, '', '#' + n.i);   // 深链接：能把「就是这个点」发给别人
  draw();
}
