...
def main():
    # 「能不能引用」从唯一真相读，不在这一页写第二遍。
    # 这一页曾经把 ai-reviewed 写死成「不能」，而 citable.json 早就把它纳进来了 ——
    # 线上两页互相打架一个多星期没人发现，因为 no-dup-defs 守的是集合字面量、
    # sync-docs 只扫 .md，两道闸都够不到 .py 生成器里的这句散文。
    import json as _j
    _c = _j.loads((ROOT / 'mappings' / 'citable.json').read_text(encoding='utf-8'))
    _CIT = set(_c['citable'])
    # 为什么是函数而不是一张表：写成 {k: … for k in ('llm-proposed','ai-reviewed', …)}
    # 时，那串档位字面量本身就是「在本地重建了一份集合」—— no-dup-defs 当场抓住了，
    # 判得对。表格的每一行本来就点了自己的档位名，不需要在这里再列一遍。
    def YN(k):
        if k not in _CIT:
            return '不能'
        return {'ai-reviewed': '能 —— 但含义是「没人挑出毛病」，不是「有人认可」',
                'ai-adjudicated': '能 —— 有过异议，已裁决',
                'expert-confirmed': '能 —— 唯一一档「有人认可」'}.get(k, '能')
...
<div class=tw><table>
<tr><th>核对到哪一步</th><th class=n>条数</th><th>能不能拿去引用</th></tr>
<tr><td>还没有人读过</td><td class=n>{c['byReview'].get('llm-proposed',0):,}</td><td class=dim>{YN('llm-proposed')}</td></tr>
<tr><td>AI 读过，没挑出毛病</td><td class=n>{c['byReview'].get('ai-reviewed',0):,}</td><td>{YN('ai-reviewed')}</td></tr>
<tr><td>AI 挑出了毛病，已挂起</td><td class=n>{c['byReview'].get('disputed',0):,}</td><td class=dim>{YN('disputed')}</td></tr>
<tr><td>判定标准客观（字表词表这类数得清的）</td><td class=n>{c['byReview'].get('auto-confirmed',0):,}</td><td>{YN('auto-confirmed')}</td></tr>
<tr><td>有过异议，已裁决</td><td class=n>{c['byReview'].get('ai-adjudicated',0):,}</td><td>{YN('ai-adjudicated')}</td></tr>
<tr><td><b>教师逐条签署</b></td><td class=n><b>{M['humanConfirmedAnchors']}</b></td><td>{YN('expert-confirmed')}</td></tr>
</table></div>
<p class=sub>「能引用」这一栏<b>不是这里写死的</b>，是从 <code>mappings/citable.json</code>
读出来的 —— 那份文件是全项目唯一定义可引用集合的地方。
这一页曾经把「AI 读过没挑出毛病」写死成「不能」，而口径早在 2026-08-20 就改了，
于是首页说 2,589 条可引用、这一页说其中 2,201 条不能，
<b>同一个站的两页互相打架</b>。散文里的判断迟早和数据分家，除非它根本不由散文来写。</p>
...
<h2>怎么用</h2>
<pre>接进你的 AI 应用（MCP，只读）
  npx --yes degit qiuyiwu1989-star/k12-knowledge-substrate k12 && node k12/mcp/server.mjs
  四个工具：查锚点 / 取全文与出处 / 走前置链 / 取分片
  每个返回都带课标出处、核对程度、粒度警告

数据集直取（允许跨域，不用 clone）
  https://k12.yongle.school/data/manifest.json
  https://k12.yongle.school/data/anchors/*.jsonl

固定到某个版本（数据只增不改，ID 永不复用）
  git clone … && git checkout v1.2
  已发布：v1.0 · v1.1 · v1.2
