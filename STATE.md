...
## 009 MCP 公开说明页（2026-09-06）
- https://k12.yongle.school/mcp/ 已上线，主页 CTA 增加“MCP 接入”。发布源码提交 32637f836246b1f9a676661f551df61a3c9ddf83。
- 只发布 site/mcp 三个静态文件并补主页链接；工作台服务、图谱数据与知识站未修改。页面本地 MCP 示例固定到 d80cb1a。
- 备份 /var/backups/k12-mcp-page/20260906T082937365273Z，含原首页与 release.json；此前无 /mcp/。回退可恢复备份首页并移走新 /mcp/ 目录，不需重启服务。
- 页面 SHA256 628e5dd6a95a6479b1b1279481016261b2fca50f0f91ce005f8e8c13a8c9cb69；公网下载比对一致，主页链接核对成功，安装脚本六个站内 URL 健康检查通过。
- npm run check 通过（同步生成日期后重跑），MCP 19 项、工作台 7 项通过；本地桌面/390 宽无横向溢出、复制配置通过。线上浏览器导航超时，线上以 HTTPS 内容校验为准。

## 008 MCP 定义（2026-09-06）
- docs/k12-mcp-v0.1.md：参考深脑两份接入契约的官方候选规范；六个 P0 工具目录与 Agent 指南，十项实施后验收场景。
- JSON 目录解析、工具唯一性、必填字段引用和 diff 检查通过；仅文档/schema 草案，未修改 MCP 运行代码，未部署，未声称完整 outputSchema 或端到端验收已完成。
- 参考深脑文档不等于实测其远程服务。MCP 官方 latest 当前为 2026-07-28，须单独实施协议支持矩阵，不能继续回显任意客户端版本。
