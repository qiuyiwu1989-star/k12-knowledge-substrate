...
const ENFORCE = {
  // 2026-08-25 升为 required：F001 清零（143 → 0）。
  // 那 143 条全是 5bed04b0「跨学段建边」带进来的 —— 那次跑了 gen_edges 落库 2,527 条，
  // **retag_edges 那一步没跟着跑完就提交了**，而没有任何东西拦得住。
  // 根因不是判据不好，是「建边」和「重标」是两步、第二步没有闸强制它跟上。
  // 所以除了升档，还加了 F006（边必须带 retagHash）—— 那是「重标真的跑过」的凭证。
  // 2026-08-25 试过升 required，退回来了：edgeTyping 这个键同时管着 F001（缺 type，
  // 该硬拦）和 W104（instrument 标 hard，本来就该是警告）。一升档，W 码被连坐成致命，
  // selftest 里「期望警告」的夹具当场挂 4 条。
  // **一个 ENFORCE 键管着语义不同的两类规则，就没法整体升档。**
  // 硬拦改由 scripts/edge-retagged.mjs 单独做：缺 type 或缺 retagHash 直接红，
  // 它只看 edges/，不跟着 validate 跑夹具。
