...
/** 保留半角标点的白名单学科。
 *  曾经把「英语」放进来 —— 错的：英语课标的能力断言本身是中文写的
 *  （「能在课堂提问或角色扮演中，与同伴围绕主题讨论」），豁免它等于
 *  让同一条断言在英语库和别科库里有两种写法，直接污染去重签名。
...
export function normalizeText(raw, { discipline = '' } = {}) {
  ...
  if (!HALFWIDTH_OK.has(discipline)) {
    ...
    // 省略号先归一，否则「...」会被逐个点转成「。。。」
    // 省略号统一成「……」。先合并各种写法，再把「……」后面粘的散点也吃掉，
    // 否则「…….4.」这种（6 个点被转成 …… 后又剩一个点）会漏网。
    s = s.replace(/[.。・]{3,}/g, '……').replace(/…{2,}/g, '……');
    s = s.replace(/…{1,}[.。]+/g, '……').replace(/[.。]+…{1,}/g, '……');
