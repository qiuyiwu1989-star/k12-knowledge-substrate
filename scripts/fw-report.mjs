...
const redundant = edges.filter((e) => reaches(e.anchorId, e.prerequisiteId, e.anchorId, e.prerequisiteId));
...
// ── W103 入度 ───────────────────────────────────────────────────────
const indeg = new Map();
for (const e of edges) indeg.set(e.anchorId, (indeg.get(e.anchorId) ?? 0) + 1);
...
L.push('## W101 传递冗余');
L.push('');
L.push(`**${redundant.length} 条 = ${(redundant.length / edges.length * 100).toFixed(0)}%**（A→B→C 已经连通，A→C 是多余的一跳）`);
L.push('');
const redBySub = {};
...
L.push('|---|---:|---:|---:|');
const edgeBySub = {};
for (const e of edges) { const d = A.get(e.anchorId).discipline; edgeBySub[d] = (edgeBySub[d] ?? 0) + 1; }
...
L.push('');
L.push(`**${far.length} 条 = ${(far.length / edges.length * 100).toFixed(1)}%**`);
L.push('');
L.push('```');
...
console.log(`  W101 传递冗余 ${redundant.length} / ${edges.length} = ${(redundant.length / edges.length * 100).toFixed(0)}%`);
