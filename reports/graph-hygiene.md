# 图体检基线报告

> 由 `npm run fw-report` 自动生成。**只报告，不设阈值，不给结论，不删任何边。**
> 存活锚点 3086 · 存活边 5057
> **推理图 3908 条**（已重标 5008 条；1149 条判为 convention，教材编排顺序不是能力依赖，移出推理图）
> 以下三项体检**只算推理图**。

## W101 传递冗余

**1111 条 = 28%**（A→B→C 已经连通，A→C 是多余的一跳）

| 学科 | 冗余边 | 该学科总边 | 占比 |
|---|---:|---:|---:|
| 化学 | 239 | 654 | 37% |
| 数学 | 196 | 574 | 34% |
| 西班牙语 | 107 | 309 | 35% |
| 通用技术 | 97 | 373 | 26% |
| 物理 | 69 | 251 | 27% |
| 地理 | 64 | 239 | 27% |
| 生物学 | 60 | 196 | 31% |
| 艺术 | 56 | 215 | 26% |
| 历史 | 46 | 151 | 30% |
| 科学 | 40 | 281 | 14% |
| 信息技术 | 32 | 105 | 30% |
| 日语 | 19 | 53 | 36% |
| 法语 | 18 | 49 | 37% |
| 道德与法治 | 15 | 65 | 23% |
| 信息科技 | 12 | 79 | 15% |
| 体育与健康 | 12 | 87 | 14% |
| 俄语 | 11 | 37 | 30% |
| 思想政治 | 9 | 48 | 19% |
| 劳动 | 7 | 26 | 27% |
| 德语 | 1 | 10 | 10% |
| 音乐 | 1 | 7 | 14% |

前 15 条（格式：`前置 → 后继` 冗余于已有通路）：

```
ca_NBdvkmXC → ca_SibDLmx3  能区分我国有代表性的地区和民族音乐的风格
ca_SxrKsY92 → ca_4Rj3vE85  能说出音乐与舞蹈、戏剧、影视在表现形式上的区别和联系
ca_SxrKsY92 → ca_n2A3hBPG  能运用所学知识和听觉经验区分中国民歌的基本体裁
ca_Y65MiK6b → ca_HueWm8JV  能根据要求，采用合适的方式进行即兴创作，完成配乐或伴奏
ca_ViFhCEpr → ca_HueWm8JV  能根据要求，采用合适的方式进行即兴创作，完成配乐或伴奏
ca_JQ2P7v3R → ca_KZnXdrmV  能构思音乐创作的主题和内容，并运用一定的技法完成富有新意的节
ca_JQ2P7v3R → ca_aeXFwdhF  能运用相关软件或其他技术，进行简单的音乐编创或编辑
ca_tYiB4tEC → ca_ZybYqDTi  能说出一段舞蹈或戏剧片段的审美特征与风格特点，并解释其表现内
ca_CLymqGRK → ca_KzbhtJPw  能对音乐在舞蹈、戏剧（含戏曲）、影视（含数字媒体艺术）等艺术
ca_SxrKsY92 → ca_MQCrMQfj  能对有代表性的世界民族民间音乐及优秀创作作品的体裁、形式、审
ca_SxrKsY92 → ca_tYiB4tEC  能对舞蹈、戏剧（含戏曲）、影视（含数字媒体艺术）等姊妹艺术中
ca_JQ2P7v3R → ca_ViFhCEpr  按要求独立或与他人合作编创、表现有重复和变化的节奏或旋律
ca_e5u7YFZu → ca_o5ghwPHL  能说出"实用与美观相结合"是设计原则
ca_6kuQkLDH → ca_E7X5tLJn  能使用图形化工具和简单的程序语言设计日常物品与居室环境
ca_fPETeiQk → ca_E7X5tLJn  能使用图形化工具和简单的程序语言设计日常物品与居室环境
```

## W102 跨学段跨度 ≥ 2 带

**68 条 = 1.7%**

```
G1–2 → G5–6  艺术｜尝试利用图形的对印在各种材质的纸上进行表现 → 能说出形状、色彩、空间是造型元素
G1–2 → G5–6  艺术｜能说出中国传统工艺是中华民族文化艺术的瑰宝 → 能说出继承与创新是传统工艺创作的重要原则
G3–4 → G7–9  艺术｜能说出"实用与美观相结合"是设计原则 → 能说出设计原则是形式、功能和社会责任相统一
G7–9 → G3–4  艺术｜能解释影视（含数字媒体艺术）是记录历史、解释 → 能说出3~7年级影视（含数字媒体艺术）的学习
G3–4 → G7–9  艺术｜能说出中国传统绘画技法是由我国历代画家不断探 → 能举例说出至少两种中国传统绘画的典型技法
G3–4 → G7–9  艺术｜能说出"实用与美观相结合"是设计原则 → 能举例说明一个设计作品如何满足实用功能的要求
G3–4 → G7–9  艺术｜能举例说明两件生活用品如何体现了实用与美观相 → 能举例说明一个设计作品如何满足实用功能的要求
G1–2 → G7–9  科学｜能仔细观察和比较，如实表达观察到的现象 → 学会根据血常规、尿常规等化验的主要结果初步判
G3–4 → G7–9  数学｜能进行整数四则混合运算（以两步为主，不超过三 → 能基于守恒和比例关系推断化学反应的相关信息
G3–4 → G7–9  数学｜能进行整数四则混合运算（以两步为主，不超过三 → 能用质量分数表示混合物体系中物质的成分
```

## W103 入度 > 8

**0 个。** 最高入度 7 —— 注意：`gen_edges.py` 本来就把候选截断在 8，所以这道闸在当前数据上**恒为 0，证明不了任何事**。真正要问的是「8 这个上限本身对不对」，而那要等 W101 处理完再看。

## 覆盖密度（学科 × 学段）

全局中位数 **31**。**不设阈值** —— 阈值要看到这份报告后由人定。

| 学科 | G1–2 | G3–4 | G5–6 | G7–9 | G10–12 | 合计 |
|---|---:|---:|---:|---:|---:|---:|
| 体育与健康 | 93 | 7 | 1 | 8 | 15 | 124 |
| 俄语 | — | — | — | — | 27 | 27 |
| 信息技术 | — | — | — | — | 67 | 67 |
| 信息科技 | 4 | 21 | 25 | 30 | — | 80 |
| 劳动 | 3 | 8 | 12 | 5 | — | 28 |
| 化学 | — | — | — | 96 | 248 | 344 |
| 历史 | — | — | — | 73 | 33 | 106 |
| 地理 | — | — | — | 44 | 114 | 158 |
| 德语 | — | — | — | — | 10 | 10 |
| 思想政治 | — | — | — | — | 46 | 46 |
| 数学 | 31 | 85 | 52 | 88 | 91 | 347 |
| 日语 | — | — | — | — | 39 | 39 |
| 法语 | — | — | — | — | 23 | 23 |
| 物理 | — | — | — | 66 | 110 | 176 |
| 生物学 | — | — | — | 71 | 67 | 138 |
| 科学 | 105 | 55 | 25 | 76 | — | 261 |
| 美术 | — | — | — | — | 9 | 9 |
| 艺术 | 18 | 16 | 19 | 68 | 44 | 165 |
| 英语 | 48 | 31 | 1 | 15 | 117 | 212 |
| 西班牙语 | — | — | — | — | 151 | 151 |
| 语文 | 105 | 10 | 11 | 113 | 24 | 263 |
| 通用技术 | — | — | — | — | 242 | 242 |
| 道德与法治 | 6 | 6 | 11 | 35 | — | 58 |
| 音乐 | — | — | — | — | 12 | 12 |

## 近重复断言（只报告，不自动合并）

实词+拉丁+数字的 Jaccard ≥ 0.92：**53 对**，其中 **15 对两条都可引用** —— 那是同一件事有了两个 ID，最该先看。

**不自动合并**：这个区间里既有真重复（只差一个括号），也有真不同
（「过去完成时／过去未完成时」「概率／条件概率」）。判据交给人。

```
1.00
   ca_xCLSAkdA [disputed] 能举例说明重要资源的主要类型、成分和用途
   ca_Naq7e63w [disputed] 能举例说明重要资源和能源的主要类型、成分和用途
1.00
   ca_Nzxb9QjR [ai-reviewed] 能解释原电池的工作原理
   ca_PMTzGcMR [disputed] 能解释电解池的工作原理
1.00 ★两条都可引用
   ca_4AcQgaPb [ai-reviewed] 能熟练地使用字典、词典独立识字
   ca_UuRKusxH [ai-reviewed] 能熟练地使用词典独立识字
1.00
   ca_qr5RchXf [disputed] 能在语篇中理解和使用动词不定式作句子中的结果状语
   ca_hCzUf94s [ai-reviewed] 能在语篇中理解和使用动词不定式作句子中的定语和结果状语
1.00 ★两条都可引用
   ca_NsGyU4hS [ai-reviewed] 能判断直线与圆 、圆与圆的位置关系
   ca_AExNhvcm [ai-reviewed] 能判断直线与圆的位置关系
1.00
   ca_taC1JpOJ [disputed] 能计算一组简单数据的离差平方和、方差
   ca_NNEhwpJC [disputed] 能计算一组简单数据的离差平方和
1.00
   ca_8fzVCSEh [ai-reviewed] 能理解与运用动词的Vている语法形式
   ca_8StDt4m3 [disputed] 能理解与运用动词的Vてある语法形式
1.00
   ca_8fzVCSEh [ai-reviewed] 能理解与运用动词的Vている语法形式
   ca_eU2k79E4 [disputed] 能理解与运用动词的Vておく语法形式
1.00
   ca_8StDt4m3 [disputed] 能理解与运用动词的Vてある语法形式
   ca_eU2k79E4 [disputed] 能理解与运用动词的Vておく语法形式
1.00
   ca_hymPp5rU [disputed] 能结合生物的生存条件以及动物与植物相互关系等知识，简单描述生物与生物、生物与环境之间相互依存的关系
   ca_8ugQh2VM [ai-reviewed] 能结合生物的生存条件以及动物与植物相互关系等知识，简单描述生物与环境之间相互依存的关系
0.96
   ca_U7GgVizO [ai-reviewed] 能说出3~4年级学生应调查不同环境中植物、动物的类型和数量
   ca_fTCqXVTg [disputed] 能说出3~4年级学生应调查不同环境中植物的类型和数量
0.96
   ca_U7GgVizO [ai-reviewed] 能说出3~4年级学生应调查不同环境中植物、动物的类型和数量
   ca_dv32R2fF [disputed] 能说出3~4年级学生应调查不同环境中动物的类型和数量
0.96
   ca_pAPgdU4g [ai-reviewed] 能在运用书面语表达的过程中，对比两种语体用词和造句的差别
   ca_dNRTSPmi [disputed] 能在运用口语和书面语表达的过程中，对比两种语体用词和造句的差别
0.96
   ca_h4wc8imp [ai-reviewed] 能在语篇中正确地理解和使用动词不定式作句子中的主语
   ca_g5EESiuV [disputed] 能在语篇中正确地理解和使用动词不定式作句子中的主语和表语
0.96
   ca_sMBqcdvq [ai-reviewed] 能在语篇中正确地理解和使用动词不定式作句子中的表语
   ca_g5EESiuV [disputed] 能在语篇中正确地理解和使用动词不定式作句子中的主语和表语
0.96
   ca_BWaQqit4 [ai-reviewed] 能选择简明、合理的表征方式描述化学变化的本质和规律
   ca_UQtCZ8DA [disputed] 能选择简明、合理的表征方式描述和说明化学变化的本质和规律
0.95 ★两条都可引用
   ca_gBE67izk [ai-reviewed] 能操作三球仪，演示出地球绕太阳公转、月球绕地球公转的相对运动
   ca_43JQQ5wK [ai-reviewed] 能操作三球仪，演示出地球绕太阳公转的相对运动
0.95
   ca_3Vf2N6Fj [disputed] 能掌握陈述式过去未完成时的变位规则和用法
   ca_UNSDrqMc [disputed] 能掌握陈述式过去完成时的变位规则和用法
0.95 ★两条都可引用
   ca_7dqYxEPA [ai-reviewed] 能运用等差数列 、等比数列解决简单的实际问题和数学问题
   ca_C2Ew4gqL [ai-reviewed] 能运用等差数列解决简单的实际问题和数学问题
0.95 ★两条都可引用
   ca_7dqYxEPA [ai-reviewed] 能运用等差数列 、等比数列解决简单的实际问题和数学问题
   ca_MxfnZh59 [ai-reviewed] 能运用等比数列解决简单的实际问题和数学问题
```
